...
helm repo update
helm install dendrite bjw-s/dendrite
```
...
```console
helm install dendrite bjw-s/dendrite -f values.yaml
```
...
| image | object | `{"pullPolicy":"IfNotPresent","repository":"ghcr.io/matrix-org/dendrite-monolith","tag":null}` |  IMPORTANT NOTE This chart inherits from our common library chart. You can check the default values/options here: https://github.com/bjw-s/library-charts/tree/main/charts/stable/common/values.yaml |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/dendrite?modal=changelog)
