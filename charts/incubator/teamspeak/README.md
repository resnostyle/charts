...
helm repo update
helm install teamspeak bjw-s/teamspeak
```
...
```console
helm install teamspeak bjw-s/teamspeak -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/teamspeak?modal=changelog)
