...
helm repo update
helm install openhab bjw-s/openhab
```
...
```console
helm install openhab bjw-s/openhab -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/openhab?modal=changelog)
