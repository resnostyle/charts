...
helm repo update
helm install torrserver bjw-s/torrserver
```
...
```console
helm install torrserver bjw-s/torrserver -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/torrserver?modal=changelog)
