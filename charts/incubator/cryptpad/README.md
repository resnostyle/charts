...
helm repo update
helm install cryptpad bjw-s/cryptpad
```
...
```console
helm install cryptpad bjw-s/cryptpad -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/cryptpad?modal=changelog)
