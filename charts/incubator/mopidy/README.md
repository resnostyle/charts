...
helm repo update
helm install mopidy bjw-s/mopidy
```
...
```console
helm install mopidy bjw-s/mopidy -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/mopidy?modal=changelog)
