...
helm repo update
helm install sinusbot bjw-s/sinusbot
```
...
```console
helm install sinusbot bjw-s/sinusbot -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/sinusbot?modal=changelog)
