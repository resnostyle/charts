...
helm repo update
helm install scrypted bjw-s/scrypted
```
...
```console
helm install scrypted bjw-s/scrypted -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/scrypted?modal=changelog)
