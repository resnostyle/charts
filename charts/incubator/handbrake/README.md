...
helm repo update
helm install handbrake bjw-s/handbrake
```
...
```console
helm install handbrake bjw-s/handbrake -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/handbrake?modal=changelog)
