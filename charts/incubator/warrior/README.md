...
helm repo update
helm install warrior bjw-s/warrior
```
...
```console
helm install warrior bjw-s/warrior -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/warrior?modal=changelog)
