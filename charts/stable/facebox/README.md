...
helm repo update
helm install facebox bjw-s/facebox
```
...
```console
helm install facebox bjw-s/facebox -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/facebox?modal=changelog)
