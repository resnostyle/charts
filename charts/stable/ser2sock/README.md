...
helm repo update
helm install ser2sock bjw-s/ser2sock
```
...
```console
helm install ser2sock bjw-s/ser2sock -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/ser2sock?modal=changelog)
