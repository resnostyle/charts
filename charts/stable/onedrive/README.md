...
helm repo update
helm install onedrive bjw-s/onedrive
```
...
```console
helm install onedrive bjw-s/onedrive -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/onedrive?modal=changelog)
