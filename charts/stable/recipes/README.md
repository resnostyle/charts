...
helm repo update
helm install recipes bjw-s/recipes
```
...
```console
helm install recipes bjw-s/recipes -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/recipes?modal=changelog)
