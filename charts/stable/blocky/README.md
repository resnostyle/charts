...
helm repo update
helm install blocky bjw-s/blocky
```
...
```console
helm install blocky bjw-s/blocky -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/blocky?modal=changelog)
