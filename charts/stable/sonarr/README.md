...
helm repo update
helm install sonarr bjw-s/sonarr
```
...
```console
helm install sonarr bjw-s/sonarr -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/sonarr"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/sonarr?modal=changelog)
