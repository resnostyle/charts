...
helm repo update
helm install neolink bjw-s/neolink
```
...
```console
helm install neolink bjw-s/neolink -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/neolink?modal=changelog)
