...
helm repo update
helm install promcord bjw-s/promcord
```
...
```console
helm install promcord bjw-s/promcord -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/promcord?modal=changelog)
