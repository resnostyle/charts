...
helm repo update
helm install cryptofolio bjw-s/cryptofolio
```
...
```console
helm install cryptofolio bjw-s/cryptofolio -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/cryptofolio?modal=changelog)
