...
helm repo update
helm install audiobookshelf bjw-s/audiobookshelf
```
...
helm install audiobookshelf \
  --set env.TZ="America/New York" \
    bjw-s/audiobookshelf
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install audiobookshelf bjw-s/audiobookshelf -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/audiobookshelf?modal=changelog)
