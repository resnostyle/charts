...
helm repo update
helm install speedtest-exporter bjw-s/speedtest-exporter
```
...
helm install speedtest-exporter \
  --set env.TZ="America/New York" \
    bjw-s/speedtest-exporter
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install speedtest-exporter bjw-s/speedtest-exporter -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/speedtest-exporter?modal=changelog)
