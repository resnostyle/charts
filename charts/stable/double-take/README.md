...
helm repo update
helm install double-take bjw-s/double-take
```
...
```console
helm install double-take bjw-s/double-take -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/double-take?modal=changelog)
