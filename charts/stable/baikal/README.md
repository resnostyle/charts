...
helm repo update
helm install baikal bjw-s/baikal
```
...
```console
helm install baikal bjw-s/baikal -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/baikal?modal=changelog)
