...
helm repo update
helm install tautulli bjw-s/tautulli
```
...
```console
helm install tautulli bjw-s/tautulli -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/tautulli"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/tautulli?modal=changelog)
