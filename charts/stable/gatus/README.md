...
helm repo update
helm install gatus bjw-s/gatus
```
...
```console
helm install gatus bjw-s/gatus -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/gatus?modal=changelog)
