...
helm repo update
helm install haste-server bjw-s/haste-server
```
...
```console
helm install haste-server bjw-s/haste-server -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/haste-server"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/haste-server?modal=changelog)
