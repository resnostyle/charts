...
helm repo update
helm install intel-gpu-plugin bjw-s/intel-gpu-plugin
```
...
helm install intel-gpu-plugin \
  --set env.TZ="America/New York" \
    bjw-s/intel-gpu-plugin
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install intel-gpu-plugin bjw-s/intel-gpu-plugin -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/intel-gpu-plugin?modal=changelog)
