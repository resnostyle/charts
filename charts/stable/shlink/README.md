...
helm repo update
helm install shlink bjw-s/shlink
```
...
```console
helm install shlink bjw-s/shlink -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/shlink?modal=changelog)
