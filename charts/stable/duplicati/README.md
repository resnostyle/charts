...
helm repo update
helm install duplicati bjw-s/duplicati
```
...
```console
helm install duplicati bjw-s/duplicati -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/duplicati?modal=changelog)
