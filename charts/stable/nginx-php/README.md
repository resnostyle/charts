...
helm repo update
helm install nginx-php bjw-s/nginx-php
```
...
```console
helm install nginx-php bjw-s/nginx-php -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/nginx-php?modal=changelog)
