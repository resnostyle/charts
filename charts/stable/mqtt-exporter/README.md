...
helm repo update
helm install mqtt-exporter bjw-s/mqtt-exporter
```
...
```console
helm install mqtt-exporter bjw-s/mqtt-exporter -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/mqtt-exporter?modal=changelog)
