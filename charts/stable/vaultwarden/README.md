...
helm repo update
helm install vaultwarden bjw-s/vaultwarden
```
...
```console
helm install vaultwarden bjw-s/vaultwarden -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/vaultwarden?modal=changelog)
