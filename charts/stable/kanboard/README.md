...
helm repo update
helm install kanboard bjw-s/kanboard
```
...
```console
helm install kanboard bjw-s/kanboard -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/kanboard?modal=changelog)
