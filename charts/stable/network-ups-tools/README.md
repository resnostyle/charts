...
helm repo update
helm install network-ups-tools bjw-s/network-ups-tools
```
...
helm install network-ups-tools \
  --set env.TZ="America/New York" \
    bjw-s/network-ups-tools
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install network-ups-tools bjw-s/network-ups-tools -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/network-ups-tools"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/network-ups-tools?modal=changelog)
