...
helm repo update
helm install amcrest2mqtt bjw-s/amcrest2mqtt
```
...
```console
helm install amcrest2mqtt bjw-s/amcrest2mqtt -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/amcrest2mqtt?modal=changelog)
