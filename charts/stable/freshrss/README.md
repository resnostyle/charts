...
helm repo update
helm install freshrss bjw-s/freshrss
```
...
```console
helm install freshrss bjw-s/freshrss -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/freshrss?modal=changelog)
