...
helm repo update
helm install smarter-device-manager bjw-s/smarter-device-manager
```
...
helm install smarter-device-manager \
  --set env.TZ="America/New York" \
    bjw-s/smarter-device-manager
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install smarter-device-manager bjw-s/smarter-device-manager -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/smarter-device-manager?modal=changelog)
