...
helm repo update
helm install magic-mirror bjw-s/magic-mirror
```
...
```console
helm install magic-mirror bjw-s/magic-mirror -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/magic-mirror?modal=changelog)
