...
helm repo update
helm install jackett bjw-s/jackett
```
...
```console
helm install jackett bjw-s/jackett -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/jackett"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/jackett?modal=changelog)
