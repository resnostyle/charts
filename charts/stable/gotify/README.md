...
helm repo update
helm install gotify bjw-s/gotify
```
...
```console
helm install gotify bjw-s/gotify -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/gotify?modal=changelog)
