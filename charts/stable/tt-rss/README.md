...
helm repo update
helm install tt-rss bjw-s/tt-rss
```
...
```console
helm install tt-rss bjw-s/tt-rss -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/tt-rss"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/tt-rss?modal=changelog)
