...
helm repo update
helm install statping bjw-s/statping
```
...
```console
helm install statping bjw-s/statping -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/statping?modal=changelog)
