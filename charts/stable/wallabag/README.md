...
helm repo update
helm install wallabag bjw-s/wallabag
```
...
```console
helm install wallabag bjw-s/wallabag -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/wallabag?modal=changelog)
