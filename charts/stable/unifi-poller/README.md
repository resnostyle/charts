...
helm repo update
helm install unifi-poller bjw-s/unifi-poller
```
...
```console
helm install unifi-poller bjw-s/unifi-poller -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/unifi-poller?modal=changelog)
