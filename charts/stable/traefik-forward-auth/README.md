...
helm repo update
helm install traefik-forward-auth bjw-s/traefik-forward-auth
```
...
helm install traefik-forward-auth \
  --set env.TZ="America/New York" \
    bjw-s/traefik-forward-auth
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install traefik-forward-auth bjw-s/traefik-forward-auth -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/traefik-forward-auth?modal=changelog)
