...
helm repo update
helm install whoogle bjw-s/whoogle
```
...
```console
helm install whoogle bjw-s/whoogle -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/whoogle?modal=changelog)
