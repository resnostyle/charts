...
helm repo update
helm install octoprint bjw-s/octoprint
```
...
```console
helm install octoprint bjw-s/octoprint -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/octoprint?modal=changelog)
