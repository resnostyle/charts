...
helm repo update
helm install photoprism bjw-s/photoprism
```
...
```console
helm install photoprism bjw-s/photoprism -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/photoprism?modal=changelog)
