...
helm repo update
helm install monica bjw-s/monica
```
...
```console
helm install monica bjw-s/monica -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/monica?modal=changelog)
