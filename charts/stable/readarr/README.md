...
helm repo update
helm install readarr bjw-s/readarr
```
...
```console
helm install readarr bjw-s/readarr -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/readarr"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/readarr?modal=changelog)
