...
helm repo update
helm install filebrowser bjw-s/filebrowser
```
...
```console
helm install filebrowser bjw-s/filebrowser -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/filebrowser?modal=changelog)
