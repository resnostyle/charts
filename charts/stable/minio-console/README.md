...
helm repo update
helm install minio-console bjw-s/minio-console
```
...
```console
helm install minio-console bjw-s/minio-console -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/minio-console?modal=changelog)
