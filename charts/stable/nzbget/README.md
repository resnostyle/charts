...
helm repo update
helm install nzbget bjw-s/nzbget
```
...
```console
helm install nzbget bjw-s/nzbget -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/nzbget"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/nzbget?modal=changelog)
