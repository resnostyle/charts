...
helm repo update
helm install pyload bjw-s/pyload
```
...
```console
helm install pyload bjw-s/pyload -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/pyload?modal=changelog)
