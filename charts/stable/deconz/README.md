...
helm repo update
helm install deconz bjw-s/deconz
```
...
```console
helm install deconz bjw-s/deconz -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/deconz?modal=changelog)
