...
helm repo update
helm install unifi bjw-s/unifi
```
...
```console
helm install unifi bjw-s/unifi -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/unifi?modal=changelog)
