...
helm repo update
helm install leaf2mqtt bjw-s/leaf2mqtt
```
...
```console
helm install leaf2mqtt bjw-s/leaf2mqtt -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/leaf2mqtt"` | Image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/leaf2mqtt?modal=changelog)
