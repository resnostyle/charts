...
helm repo update
helm install apache-musicindex bjw-s/apache-musicindex
```
...
helm install apache-musicindex \
  --set env.TZ="America/New York" \
    bjw-s/apache-musicindex
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install apache-musicindex bjw-s/apache-musicindex -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/apache-musicindex"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/apache-musicindex?modal=changelog)
