...
helm repo update
helm install wikijs bjw-s/wikijs
```
...
```console
helm install wikijs bjw-s/wikijs -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/wikijs?modal=changelog)
