...
helm repo update
helm install nightscout bjw-s/nightscout
```
...
```console
helm install nightscout bjw-s/nightscout -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/nightscout?modal=changelog)
