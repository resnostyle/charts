...
helm repo update
helm install wireguard bjw-s/wireguard
```
...
```console
helm install wireguard bjw-s/wireguard -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/wireguard"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/wireguard?modal=changelog)
