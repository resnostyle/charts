...
helm repo update
helm install alertmanager-bot bjw-s/alertmanager-bot
```
...
helm install alertmanager-bot \
  --set env.TZ="America/New York" \
    bjw-s/alertmanager-bot
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install alertmanager-bot bjw-s/alertmanager-bot -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/alertmanager-bot?modal=changelog)
