...
helm repo update
helm install stash bjw-s/stash
```
...
```console
helm install stash bjw-s/stash -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/stash?modal=changelog)
