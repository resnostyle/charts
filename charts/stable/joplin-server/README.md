...
helm repo update
helm install joplin-server bjw-s/joplin-server
```
...
```console
helm install joplin-server bjw-s/joplin-server -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/joplin-server?modal=changelog)
