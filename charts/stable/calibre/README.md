...
helm repo update
helm install calibre bjw-s/calibre
```
...
```console
helm install calibre bjw-s/calibre -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/calibre?modal=changelog)
