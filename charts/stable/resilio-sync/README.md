...
helm repo update
helm install resilio-sync bjw-s/resilio-sync
```
...
```console
helm install resilio-sync bjw-s/resilio-sync -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/resilio-sync?modal=changelog)
