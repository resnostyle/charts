...
helm repo update
helm install powerdns-admin bjw-s/powerdns-admin
```
...
helm install powerdns-admin \
  --set env.TZ="America/New York" \
    bjw-s/powerdns-admin
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install powerdns-admin bjw-s/powerdns-admin -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/powerdns-admin?modal=changelog)
