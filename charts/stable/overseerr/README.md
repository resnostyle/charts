...
helm repo update
helm install overseerr bjw-s/overseerr
```
...
```console
helm install overseerr bjw-s/overseerr -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/overseerr?modal=changelog)
