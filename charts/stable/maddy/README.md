...
helm repo update
helm install maddy bjw-s/maddy
```
...
```console
helm install maddy bjw-s/maddy -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/maddy?modal=changelog)
