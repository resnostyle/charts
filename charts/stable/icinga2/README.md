...
helm repo update
helm install icinga2 bjw-s/icinga2
```
...
```console
helm install icinga2 bjw-s/icinga2 -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/icinga2?modal=changelog)
