...
helm repo update
helm install puppeteer bjw-s/puppeteer
```
...
```console
helm install puppeteer bjw-s/puppeteer -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/puppeteer?modal=changelog)
