...
helm repo update
helm install radarr bjw-s/radarr
```
...
```console
helm install radarr bjw-s/radarr -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/radarr"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/radarr?modal=changelog)
