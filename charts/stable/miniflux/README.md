...
helm repo update
helm install miniflux bjw-s/miniflux
```
...
```console
helm install miniflux bjw-s/miniflux -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/miniflux?modal=changelog)
