...
helm repo update
helm install seafile bjw-s/seafile
```
...
```console
helm install seafile bjw-s/seafile -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/seafile?modal=changelog)
