...
helm repo update
helm install deemix bjw-s/deemix
```
...
```console
helm install deemix bjw-s/deemix -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/deemix?modal=changelog)
