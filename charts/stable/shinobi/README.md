...
helm repo update
helm install shinobi bjw-s/shinobi
```
...
```console
helm install shinobi bjw-s/shinobi -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/shinobi?modal=changelog)
