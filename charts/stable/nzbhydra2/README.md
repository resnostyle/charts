...
helm repo update
helm install nzbhydra2 bjw-s/nzbhydra2
```
...
```console
helm install nzbhydra2 bjw-s/nzbhydra2 -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/nzbhydra2"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/nzbhydra2?modal=changelog)
