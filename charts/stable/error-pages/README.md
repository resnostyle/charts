...
helm repo update
helm install error-pages bjw-s/error-pages
```
...
```console
helm install error-pages bjw-s/error-pages -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/error-pages?modal=changelog)
