...
helm repo update
helm install lidarr bjw-s/lidarr
```
...
```console
helm install lidarr bjw-s/lidarr -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/lidarr"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/lidarr?modal=changelog)
