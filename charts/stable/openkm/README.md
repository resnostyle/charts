...
helm repo update
helm install openkm bjw-s/openkm
```
...
```console
helm install openkm bjw-s/openkm -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/openkm?modal=changelog)
