...
helm repo update
helm install rhasspy bjw-s/rhasspy
```
...
```console
helm install rhasspy bjw-s/rhasspy -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/rhasspy?modal=changelog)
