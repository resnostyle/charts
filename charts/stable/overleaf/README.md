...
helm repo update
helm install overleaf bjw-s/overleaf
```
...
```console
helm install overleaf bjw-s/overleaf -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/overleaf?modal=changelog)
