...
helm repo update
helm install sharry bjw-s/sharry
```
...
```console
helm install sharry bjw-s/sharry -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/sharry?modal=changelog)
