...
helm repo update
helm install navidrome bjw-s/navidrome
```
...
```console
helm install navidrome bjw-s/navidrome -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/navidrome?modal=changelog)
