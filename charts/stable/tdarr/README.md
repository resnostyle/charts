...
helm repo update
helm install tdarr bjw-s/tdarr
```
...
```console
helm install tdarr bjw-s/tdarr -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/tdarr?modal=changelog)
