...
helm repo update
helm install zigbee2mqtt bjw-s/zigbee2mqtt
```
...
```console
helm install zigbee2mqtt bjw-s/zigbee2mqtt -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/zigbee2mqtt?modal=changelog)
