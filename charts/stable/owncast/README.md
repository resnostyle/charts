...
helm repo update
helm install owncast bjw-s/owncast
```
...
```console
helm install owncast bjw-s/owncast -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/owncast?modal=changelog)
