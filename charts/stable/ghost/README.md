...
helm repo update
helm install ghost bjw-s/ghost
```
...
```console
helm install ghost bjw-s/ghost -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/ghost?modal=changelog)
