...
helm repo update
helm install influxdb-exporter bjw-s/influxdb-exporter
```
...
helm install influxdb-exporter \
  --set env.TZ="America/New York" \
    bjw-s/influxdb-exporter
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install influxdb-exporter bjw-s/influxdb-exporter -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/influxdb-exporter?modal=changelog)
