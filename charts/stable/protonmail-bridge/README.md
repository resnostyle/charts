...
helm repo update
helm install protonmail-bridge bjw-s/protonmail-bridge
```
...
helm install protonmail-bridge \
  --set env.TZ="America/New York" \
    bjw-s/protonmail-bridge
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install protonmail-bridge bjw-s/protonmail-bridge -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/protonmail-bridge?modal=changelog)
