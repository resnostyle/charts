...
helm repo update
helm install healthchecks bjw-s/healthchecks
```
...
```console
helm install healthchecks bjw-s/healthchecks -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/healthchecks?modal=changelog)
