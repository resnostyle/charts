...
helm repo update
helm install rtorrent-flood bjw-s/rtorrent-flood
```
...
helm install rtorrent-flood \
  --set env.TZ="America/New York" \
    bjw-s/rtorrent-flood
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install rtorrent-flood bjw-s/rtorrent-flood -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/rtorrent-flood?modal=changelog)
