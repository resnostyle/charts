...
helm repo update
helm install rtorrent-rutorrent bjw-s/rtorrent-rutorrent
```
...
helm install rtorrent-rutorrent \
  --set env.TZ="America/New York" \
    bjw-s/rtorrent-rutorrent
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install rtorrent-rutorrent bjw-s/rtorrent-rutorrent -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/rtorrent-rutorrent?modal=changelog)
