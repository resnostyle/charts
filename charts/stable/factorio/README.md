...
helm repo update
helm install factorio bjw-s/factorio
```
...
```console
helm install factorio bjw-s/factorio -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/factorio?modal=changelog)
