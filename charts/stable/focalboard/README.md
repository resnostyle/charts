...
helm repo update
helm install focalboard bjw-s/focalboard
```
...
```console
helm install focalboard bjw-s/focalboard -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/focalboard?modal=changelog)
