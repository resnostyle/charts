...
helm repo update
helm install privatebin bjw-s/privatebin
```
...
```console
helm install privatebin bjw-s/privatebin -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/privatebin?modal=changelog)
