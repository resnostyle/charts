...
helm repo update
helm install mealie bjw-s/mealie
```
...
```console
helm install mealie bjw-s/mealie -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/mealie?modal=changelog)
