...
helm repo update
helm install unpackerr bjw-s/unpackerr
```
...
```console
helm install unpackerr bjw-s/unpackerr -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/unpackerr?modal=changelog)
