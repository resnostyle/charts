...
helm repo update
helm install bazarr bjw-s/bazarr
```
...
```console
helm install bazarr bjw-s/bazarr -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/bazarr"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/bazarr?modal=changelog)
