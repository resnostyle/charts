...
helm repo update
helm install dokuwiki bjw-s/dokuwiki
```
...
```console
helm install dokuwiki bjw-s/dokuwiki -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/dokuwiki?modal=changelog)
