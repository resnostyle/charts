...
helm repo update
helm install frigate bjw-s/frigate
```
...
```console
helm install frigate bjw-s/frigate -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/frigate?modal=changelog)
