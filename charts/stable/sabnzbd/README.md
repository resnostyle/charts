...
helm repo update
helm install sabnzbd bjw-s/sabnzbd
```
...
```console
helm install sabnzbd bjw-s/sabnzbd -f values.yaml
```
...
| image.repository | string | `"ghcr.io/bjw-s/sabnzbd"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/bjw-s/sabnzbd?modal=changelog)
